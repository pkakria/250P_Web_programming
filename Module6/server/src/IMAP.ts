...
// for e.g. all functions list messages, retrieve a message, list mailboxes and delete messages
// use mailbox name. Deleting and listing a message also require id. So we 
// declare id as optionsl
...
export class Worker {
  ...
  public async listMessages(inCallOptions: ICallOptions): Promise<IMessage[]> {
    ...
    const messages: any[] = await client.listMessages(
      ...
      "1:*", // get all messages from first onwards
      [ "uid", "envelope" ] // unique id of message and the metadata (envelope)
    );
  ...
  public async getMessageBody(inCallOptions: ICallOptions): Promise<string> {
    ...
    await client.close();
    return ""+ parsed.text; // added "" because parsed.text might be undefined
